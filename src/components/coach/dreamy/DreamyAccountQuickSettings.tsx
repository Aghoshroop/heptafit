      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-4 gap-4">
          {settings.map((setting, i) => (
            <div key={i} className="bg-muted/50 border border-border rounded-xl p-4 flex items-center justify-between group cursor-pointer hover:bg-accent/60 transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-xs font-semibold text-foreground">{setting.name}</h4>
                  {setting.status ? (
                    <span className="text-[9px] font-medium text-[#34D399]">{setting.status}</span>
                  ) : (
                    <p className="text-[9px] text-muted-foreground mt-0.5">{setting.desc}</p>
                  )}
                </div>
              </div>
