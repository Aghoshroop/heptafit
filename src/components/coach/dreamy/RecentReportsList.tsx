  ...
  return (
    <div className="bg-[#11141A] rounded-xl p-5 border border-[#1F2937] flex-1 flex flex-col min-h-0">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar space-y-3">
        {reports.length === 0 ? (
          <div className="text-center text-[#6B7280] text-xs py-6">No recent reports found.</div>
        ) : (
          reports.map((report, i) => (
            <div key={i} className="flex gap-3 items-center group cursor-pointer hover:bg-[#1F2937]/50 p-2 rounded-lg -mx-2 transition-colors">
              <div className="w-8 h-8 rounded bg-[#1F2937] flex items-center justify-center shrink-0">
                <FileText size={14} className="text-[#F59E0B]" />
              </div>
              <div className="flex-1 min-w-0">
                <h4 className="text-xs font-semibold text-white truncate group-hover:text-[#8B5CF6] transition-colors">{report.title}</h4>
                <p className="text-[10px] text-[#9CA3AF] mt-0.5">{report.time}</p>
              </div>
              <button className="w-6 h-6 rounded flex items-center justify-center text-[#9CA3AF] hover:text-white hover:bg-[#1F2937] transition-all">
                <Download size={14} />
              </button>
            </div>
          ))
