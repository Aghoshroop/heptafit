...
import { 
  ...
  Home, Dumbbell, Heart, Ruler, Trophy, Calendar, 
...
const studentNavItems = [
  { name: "Dashboard", href: "/athlete", icon: Home },
  { name: "My Training", href: "/athlete/training", icon: Dumbbell },
  { name: "My Performance", href: "/athlete/performance", icon: Activity },
  { name: "My Wellness", href: "/athlete/wellness", icon: Heart },
  { name: "My Body Metrics", href: "/athlete/metrics", icon: Ruler },
  { name: "My Competitions", href: "/athlete/competitions", icon: Trophy },
  { name: "My Calendar", href: "/athlete/calendar", icon: Calendar },
  { name: "Coach Notes", href: "/athlete/coach-notes", icon: ClipboardList },
  { name: "Team Feed", href: "/athlete/feed", icon: MessageSquare },
  { name: "Documents", href: "/athlete/documents", icon: FolderOpen },
  { name: "Messages", href: "/athlete/messages", icon: Mail },
...
export function StudentAppLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.error("StudentAppLayout notifications onSnapshot error:", error);
    });
    return () => unsubscribe();
  }, [user]);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex relative">
      {/* Ambient Background Glows tailored for Student */}
      <div className="ambient-glow bg-emerald-500/10 w-[600px] h-[600px] top-[-200px] left-[-200px]" />
      <div className="ambient-glow bg-teal-500/10 w-[800px] h-[800px] bottom-[-400px] right-[-200px]" />
              ...
              <Link key={item.name} href={item.href} className="block relative">
                {isActive && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-gradient-to-r from-emerald-500/20 to-transparent border-l-2 border-emerald-500 rounded-r-xl"
                    initial={false}
            ...
            <Link href="/athlete/profile" className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-white/5 transition-colors cursor-pointer group">
              ...
              <div className="flex-1 overflow-hidden">
                <p className="text-sm font-semibold truncate text-foreground group-hover:text-emerald-500 transition-colors">
                  {userData?.firstName || "Athlete"} {userData?.lastName}
                </p>
                <p className="text-xs text-muted-foreground capitalize">My Profile</p>
              </div>
      ...
      <main className="flex-1 flex flex-col min-h-screen relative z-10 min-w-0">
        <header className="h-20 lg:h-24 px-4 lg:px-8 flex items-center justify-between sticky top-0 z-30 bg-background/80 backdrop-blur-xl border-b border-white/5">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-bold hidden md:block">Athlete Portal</h2>
          </div>
