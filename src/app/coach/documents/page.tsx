...
export default function CoachDocumentsPage() {
  ...
  const handleUpload = async (url: string, name: string, type: string) => {
    try {
      ...
      toast.success("Team document uploaded successfully");
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Team Documents</h1>
        <p className="text-muted-foreground mt-1">Manage all team waivers, playbooks, and medical files.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card glass className="lg:col-span-1 h-fit">
          <CardHeader>
            <CardTitle>Upload Team Document</CardTitle>
            <CardDescription>Upload global resources for the team.</CardDescription>
          </CardHeader>
          <CardContent>
            <FileUpload 
                  ...
                  <CardContent className="p-4 flex items-start gap-4">
                    ...
                    <div className="flex-1 overflow-hidden">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {doc.athleteId === 'global' ? 'Team Resource' : (athletes[doc.athleteId] || 'Unknown Athlete')}
                      </p>
