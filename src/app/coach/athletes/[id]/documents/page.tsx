...
export default function DocumentsPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    await addDoc(collection(db, "users", athleteId, "documents"), {
      title: newItem,
  ...
  const handleDelete = async (id: string) => {
    await deleteDoc(doc(db, "users", athleteId, "documents", id));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleAdd} className="flex gap-2">
        <Input 
          placeholder="Add a new entry for Documents..." 
          value={newItem}
      ...
      {loading ? (
        ...
        <div className="bg-background/30 border border-white/5 rounded-xl p-8 text-center">
          <p className="text-muted-foreground">No records found for Documents.</p>
        </div>
      ) : (
