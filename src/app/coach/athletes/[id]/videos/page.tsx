...
export default function VideosPage() {
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "users", athleteId, "videos"),
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    await addDoc(collection(db, "users", athleteId, "videos"), {
      title: newItem,
  ...
  const handleDelete = async (id: string) => {
    await deleteDoc(doc(db, "users", athleteId, "videos", id));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleAdd} className="flex gap-2">
        <Input 
          placeholder="Add a new entry for Videos..." 
          value={newItem}
      ...
      {loading ? (
        ...
        <div className="bg-background/30 border border-white/5 rounded-xl p-8 text-center">
          <p className="text-muted-foreground">No records found for Videos.</p>
        </div>
      ) : (
