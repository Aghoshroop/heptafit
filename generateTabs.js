...
const tabs = [
  'personal', 'medical', 'metrics', 'wellness', 'performance', 
  'training', 'competitions', 'attendance', 'coach-notes', 
  'nutrition', 'injuries', 'documents', 'videos', 'reports', 'timeline'
];

const basePath = path.join(process.cwd(), 'src', 'app', 'coach', 'athletes', '[id]');

tabs.forEach(tab => {
  const dirPath = path.join(basePath, tab);
  if (!fs.existsSync(dirPath)) {
    fs.mkdirSync(dirPath, { recursive: true });
  }
  
  const title = tab.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
  const collectionName = tab.replace('-', '_'); // e.g. coach_notes
...
export default function ${title.replace(' ', '')}Page() {
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, "users", athleteId, "${collectionName}"),
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    await addDoc(collection(db, "users", athleteId, "${collectionName}"), {
      title: newItem,
  ...
  const handleDelete = async (id: string) => {
    await deleteDoc(doc(db, "users", athleteId, "${collectionName}", id));
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleAdd} className="flex gap-2">
        <Input 
          placeholder="Add a new entry for ${title}..." 
          value={newItem}
      ...
      {loading ? (
        ...
        <div className="bg-background/30 border border-white/5 rounded-xl p-8 text-center">
          <p className="text-muted-foreground">No records found for ${title}.</p>
        </div>
      ) : (
